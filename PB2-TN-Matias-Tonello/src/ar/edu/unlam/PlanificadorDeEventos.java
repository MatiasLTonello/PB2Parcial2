...
public class PlanificadorDeEventos {
    ...
    public void crear(Usuario organizador, Evento evento) {
        ...
        eventos.add(cumple);
        cumple.agregarInvitado(organizador);
    ...
    public void crear(Usuario organizador, String nombreEvento) {
        ...
        casamiento.agregarInvitado(organizador);
       	eventos.add(casamiento);
       	usuarios.add(organizador);
    }

	public Boolean invitar(Evento evento, Usuario usuario) throws UsuarioInexistente {
		Usuario usuarioAInvitar = evento.getUsuario(usuario.getEmail());
		if(usuarioAInvitar != null)
			return evento.agregarInvitado(usuarioAInvitar);
	...
	public Integer getCantidadDeInvitadosConfirmados() {
		...
		for(Evento evento: eventos) {
			confirmados = confirmados + evento.getInvitadosConfirmados();
		}
		return confirmados;
	}
